...
class GuiWorker:
    ...
    def execute_task(self, task, button, label):
        try:
            ...
            self.logger.error(f"Failed to do execute the task [{job_type}]. Error Details >> {ex}")
    ...
    def cleanup(self, group):
        try:
            ...
            ex = traceback.format_exc()
            self.logger.error(f"Failed to do cleanup the task [{group.task_name}]. Error Details >> {ex}")
    ...
    def sensor_feedback_update(self, sensor):
        try:
            ...
            ex = traceback.format_exc()
            self.logger.error(f"Failed to provide sensor feedback update. Error Details >> {ex}")
